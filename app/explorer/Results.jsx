    ...
    const selectedItems = this.props.selectedFeatures.map((feature) => {
      switch (feature.layer.source) {
        case 'capital-projects':
          return <CPListItem feature={feature} key={`cp${feature.properties.maprojid}`} />;
        case 'sca-points':
          return <SCAListItem feature={feature} key={`sca${feature.properties.cartodb_id}`} />;
        case 'facilities-cp':
          return <FacilitiesListItem feature={feature} key={`fac${feature.properties.uid}`} />;
        case 'housing-development':
          return <HousingDevelopmentListItem feature={feature} key={`dev${feature.properties.dob_job_number}`} />;
        case 'cb-budgetrequests':
          return <BudgetRequestLineItem feature={feature} key={`cbbr${feature.properties.regid}`} />;
        default:
          return null;
      }
    });
