...
The script is returning two extra housing records that are excluded from the UI, which needs to be manually corrected.  If you run the totalcounts script, it will overwrite this change.  You should make sure that all of the values are correct by checking that when no filters are enabled, the UI shows `Showing all XXXXX records`, and not `XXXXX of YYYYY records`.
